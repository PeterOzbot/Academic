...
public class Interpreter {
	...
	public Interpreter(Frame frame, ImSEQ code) {
		...
		fp = sp;
		sp = sp - frame.getSize() - 4;// TODO: -4 da je vkljuèen SL = ocitno
										// prej nesme bit?
		if (debug) {
			...
			if (result instanceof Label) {
				for (pc = 0; pc < code.codes.size(); pc++) {
					...
					if ((instruction instanceof ImLABEL)
							&& (((ImLABEL) instruction).label.label
									.equals(((Label) result).label)))
						break;
				}
			} else
				pc++;
	...
	public Object execute(ImCode instruction) {
		...
		if (instruction instanceof ImBINOP) {
			...
			switch (instr.oper) {
			case ImBINOP.OR:
				return ((((Integer) fstSubValue).intValue() != 0)
						|| (((Integer) sndSubValue).intValue() != 0) ? 1 : 0);
			case ImBINOP.AND:
				return ((((Integer) fstSubValue).intValue() != 0)
						&& (((Integer) sndSubValue).intValue() != 0) ? 1 : 0);
			case ImBINOP.EQUi:
				return (((Integer) fstSubValue).intValue() == ((Integer) sndSubValue)
						.intValue() ? 1 : 0);
			case ImBINOP.NEQi:
				return (((Integer) fstSubValue).intValue() != ((Integer) sndSubValue)
						.intValue() ? 1 : 0);
			...
			case ImBINOP.MODi:
				return (((Integer) fstSubValue).intValue() % ((Integer) sndSubValue)
						.intValue());
			case ImBINOP.EQUr:
				return (((Float) fstSubValue).floatValue() == ((Float) sndSubValue)
						.floatValue() ? 1 : 0);
			case ImBINOP.NEQr:
				return (((Float) fstSubValue).floatValue() != ((Float) sndSubValue)
						.floatValue() ? 1 : 0);
			...
			case ImBINOP.MODr:
				return (((Float) fstSubValue).floatValue() % ((Float) sndSubValue)
						.floatValue());
			case ImBINOP.EQUs:
				return (((String) fstSubValue).compareTo((String) sndSubValue)) == 0 ? 1
						: 0;
			case ImBINOP.NEQs:
				return (((String) fstSubValue).compareTo((String) sndSubValue)) != 0 ? 1
						: 0;
		...
		if (instruction instanceof ImCALL) {
			...
			if (instr.fun.label.equals("Lsys::getString")) {
				...
				stM((Integer) ldM(sp + 4),value);
				scanner.close();
				return null;
			}
			
			new Interpreter(
					compiler.lincode.CodeGenerator.framesByLabel.get(instr.fun),
					(ImSEQ) compiler.lincode.CodeGenerator.codesByLabel
							.get(instr.fun));
		...
		if (instruction instanceof ImCJUMP) {
			...
			if (cond instanceof Integer) {
				...
				Report.error("CJUMP: illegal condition type.", 1);
		}
		...
		if (instruction instanceof ImNAME) {
			...
			if (instr.name.equals("FP"))
				return fp;
			if (instr.name.equals("SP"))
				return sp;
		}
